...
export class CACSignatureManager {
  ...
  private static async insertSignature(
    tx: TxDb,
    requestId: number,
    signerId: number,
    stepType: string,
    ...
    try {
      const result = await db.withTransaction(async (tx) => {
        ...
        if (!certValid.isValid) {
          return { success: false as const, error: certValid.error || 'Certificate validation failed' };
        }

        const stepType = role === 'CPSO' ? 'cpso_approval' : 'approver_approval';
        const signatureId = await this.insertSignature(tx, requestId, signerId, stepType, signatureData);
  ...
  static async applySignature(
    ...
    try {
      const result = await db.withTransaction(async (tx) => {
        ...
        const signatureId = await this.insertSignature(tx, requestId, signerId, 'requestor_signature', signatureData);
  ...
  static async applyDTASignature(
    ...
    try {
      const result = await db.withTransaction(async (tx) => {
        ...
        const signatureId = await this.insertSignature(tx, requestId, signerId, 'dta_signature', signatureData);
  ...
  static async applySMESignature(
    ...
    try {
      const result = await db.withTransaction(async (tx) => {
        ...
        if (!request) {
          return { success: false as const, error: 'Request not found or not ready for SME signature' };
        }
        ...
        const signatureId = await this.insertSignature(tx, requestId, signerId, 'sme_signature', signatureData);
  ...
  static async applyDTASignatureOnly(
    ...
    try {
      const result = await db.withTransaction(async (tx) => {
        ...
        const signatureId = await this.insertSignature(tx, requestId, signerId, 'dta_signature_only', signatureData);
  ...
  static async getRequestSignatures(requestId: number): Promise<any[]> {
    const db = getDb();
  ...
  static async exportSignatureData(signatureId: number): Promise<any> {
    const db = getDb();
