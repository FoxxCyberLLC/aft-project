// Approver All Requests Page (shows all users' requests for review and oversight)
...
export class ApproverAllRequests {
  static async render(user: ApproverUser, viewMode: 'table' | 'timeline' = 'table'): Promise<string> {
    const db = getDb();
    ...
    const columns = [
      ...
      },
      {
        ...
        render: (value: any, row: any) => ComponentBuilder.tableCellActions([
          ...
          { label: 'Details', onClick: `viewRequest(${row.id})`, variant: 'secondary' }
        ])
      }
    ];
    ...
    const actions = ComponentBuilder.tableActions({
      secondary: [
        ...
        { label: 'Refresh', onClick: 'refreshRequests()' }
      ]
    });
    ...
    const tableContainer = ComponentBuilder.tableContainer({
      ...
      description: 'Review and oversee all Assured File Transfer requests across the organization',
      search,
      filters,
      actions: `
            ...
            <div class="bg-[var(--card)] border border-[var(--border)] rounded-lg p-4 hover:shadow-md transition-shadow">
              <div class="flex items-center justify-between mb-3">
                ...
                <div class="flex gap-1">
                  <button onclick="reviewRequest(${request.id})" class="action-btn primary" style="padding: 0.25rem 0.5rem; font-size: 0.75rem;">Review</button>
                  <button onclick="viewRequest(${request.id})" class="action-btn secondary" style="padding: 0.25rem 0.5rem; font-size: 0.75rem;">Details</button>
                </div>
              </div>
