...
async function render(user: ApproverUser, requestId: string): Promise<string> {
  const db = getDb();
...
function renderApprovalActions(request: any): string {
  ...
  if (
    ['approved', 'rejected', 'completed', 'cancelled', 'pending_cpso', 'pending_dta'].includes(
      request.status,
    )
  ) {
    let statusMessage = '';
...
function renderNotFound(user: ApproverUser): string {
  const content = `
