            ...
            This request has already been ${
              request.status === 'approved'
                ...
                : request.status === 'pending_dta'
                  ? 'approved by CPSO and forwarded to DTA'
                  : request.status === 'active_transfer'
                    ? 'approved and is in active transfer'
                    : request.status
            }.
          </p>
...
function renderNotFound(user: CPSOUser): string {
  const content = `
  ...
  return CPSONavigation.renderLayout('Request Not Found', '', user, '/cpso/request', content);
}
