// CPSO All Requests Page (shows all users' requests for CPSO review and oversight)
...
async function render(user: CPSOUser, viewMode: 'table' | 'timeline' = 'table'): Promise<string> {
  const db = getDb();
          ...
          <div class="bg-[var(--card)] border border-[var(--border)] rounded-lg p-4 hover:shadow-md transition-shadow">
            <div class="flex items-center justify-between mb-3">
              ...
              <div class="flex gap-1">
                <button onclick="viewRequest(${request.id})" class="action-btn primary" style="padding: 0.25rem 0.5rem; font-size: 0.75rem;">Review</button>
